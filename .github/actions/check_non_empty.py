...
# Function to check for non-empty strings in each language column for each row
def check_non_empty_strings(event_prefix):
    ...
    with open(os.environ['GITHUB_STEP_SUMMARY'], 'w') as summary_file:
        summary_file.write(f"## Summary of Non-Empty Strings Check\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python check_non_empty.py <event_prefix>")
        sys.exit(1)

    event_prefix = sys.argv[1]
    check_non_empty_strings(event_prefix)
